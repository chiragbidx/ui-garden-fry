...
const DEFAULT_MAX_MIGRATE_ATTEMPTS = 8;
...
const MAX_MIGRATE_ATTEMPTS = parsePositiveInt(
  process.env.DB_MIGRATE_ATTEMPTS,
  DEFAULT_MAX_MIGRATE_ATTEMPTS
);
...
function runDrizzleMigrate() {
  if (fs.existsSync(LOCAL_DRIZZLE_BIN)) {
    run(LOCAL_DRIZZLE_BIN, ["migrate", "--config", "drizzle.config.ts"]);
    return;
  }

  // Fallback when local binary is missing; do not pin an old CLI version.
  run("npx", ["--yes", "drizzle-kit", "migrate", "--config", "drizzle.config.ts"]);
}
  ...
  try {
    ...
    if (!isTransientDbError(error) || isLastAttempt) {
      if (isLastAttempt && isTransientDbError(error)) {
        console.error(
          "❌ Migration failed after all retry attempts. Verify DB reachability and TLS settings (set DATABASE_SSL=true if your provider requires SSL)."
        );
      }
      throw error;
    }
